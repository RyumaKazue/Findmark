...
- 左ペイン(220px固定): 常時表示のフォルダツリー。「絞り込み指定」と「ドロップ先」の2役。各フォルダ行の右端には**フォルダ選択チップ**（コンパクトなラジオ風トグル18px。押下でそのフォルダ配下=サブフォルダ含む を右ペインに表示。選択中は accent 塗り＋チェック）を置く。展開/折りたたみは「三角＋📁＋フォルダ名」の押下範囲で行う。※デザインmockはこの位置に件数を表示するが、**件数に代えて選択チップを置く**（ユーザー指示による意図的逸脱。U7 で実装、フル機能=検索ボックスへのチップ挿入・直下トグル・双方向同期は U11）。
- 深い階層のフォルダ名は末尾省略（…）＋**全文ツールチップ**で表示し、インデントを深いほど詰めて名前幅を確保する（U7）。子リスト省略「さらにN件」・パス圧縮・「階層をたたむ」バー等の本格対応は **U11（状態2b）**。
...
| 状態 | 内容 | 実装作業単位 |
|---|---|---|
| 1a | 通常（検索前・3領域シェル・最近のブックマーク表示） | U7 |
| 1b | フォルダ絞り込み中（検索ボックス先頭にフォルダチップ） | U11 |
