...
> | 種別 | 対象 | 有効なモード | 例 |
> |---|---|---|---|
> | **行に紐づく操作** | フォーカス中の結果行 | `LIST` のみ | `F2`/`Ctrl+E`、`Ctrl+;`、`Ctrl+M`(選択なし=単一移動)、`Delete`(選択なし=単一削除) |
> | **行に紐づかない操作** | チェック選択 / 対象なし | `LIST` + `FOLDER_TREE` | `Ctrl+D`(現在ページ登録)、`Ctrl+M`(選択あり=一括移動)、`Ctrl/Cmd+Z`(アンドゥ)、`Escape`(選択解除) |
>
> 後者は左ペインにフォーカスがあっても動作しなければならない。U11 で**起動直後の既定フォーカスが左ペイン(`FOLDER_TREE`)になった**ため、これを `LIST` 限定にすると「ポップアップを開いた直後に効かない」という主要導線の欠落になる(実際に `Ctrl+D`・一括移動で発生していた)。実装上は `FOLDER_TREE` から `exitToList()` を経由して `PANEL` へ入るため、上図の遷移規則自体は維持される。
