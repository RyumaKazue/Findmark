...
- 左ペイン(220px固定): 常時表示のフォルダツリー。「絞り込み指定」と「ドロップ先」の2役。視認性のためフォルダ📁・名前はやや大きめ（名前14px / 📁16px / 行高34px）。各フォルダ行の右端には**フォルダ選択ボタン（📎クリップ）**（押下でそのフォルダ配下=サブフォルダ含む を右ペインに表示。選択状態は強調せず「押すためのボタン」に徹する。横スクロール時も右端に固定表示）を置く。※デザインmockはこの位置に件数を表示するが、**件数に代えて📎選択ボタンを置く**（ユーザー指示による意図的逸脱。U7 で実装、フル機能=検索ボックスへのチップ挿入・直下トグル・双方向同期は U11）。
