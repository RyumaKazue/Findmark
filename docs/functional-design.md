...
**構成方針**:
- **バックグラウンド(Service Worker)は薄く保つ**。検索・編集はPopupのUIスレッドで完結し、Service Workerはブラウザ起動時の掃除処理(存在しないフォルダID/別名参照/期限切れゴミ箱のクリーンアップ)と起動ショートカットの割り当て検証のみを担う。イベント登録と既存レイヤーの呼び出しに徹し、判定ロジックは最小契約DIで注入してユニットテスト可能に保つ(詳細は [architecture.md](./architecture.md)「背景(Service Worker)」)。
...
interface LocalState {           // storage.local(端末固有・sync不可)
  ...
  session?: PopupSession;        // ポップアップ状態の復元(機能13)
}

/**
 * ポップアップを閉じた時点の UI 状態。次回起動時に復元する(機能13)。
 * 未保存(初回起動)の場合の既定値は下記コメントのとおり。
 */
interface PopupSession {
  focusArea: 'search' | 'result' | 'folderTree';  // 既定: 'folderTree'
  scopeFolderId: string | null;                   // 既定: null(=「すべて」)
  selectedBookmarkId?: string;                    // 既定: 未指定(=先頭行)。ID で保持しインデックスでは持たない
  query: string;                                  // 既定: ''
}
```

**復元時のフォールバック**: `scopeFolderId` のフォルダや `selectedBookmarkId` のブックマークが Chrome 標準機能等で削除されている場合、**当該項目のみ既定値へ倒す**(スコープ→「すべて」/ 選択行→先頭行)。`focusArea` は影響を受けず保存値のまま復元する。
...
sequenceDiagram
    ...
    Popup->>Popup: 既定フォーカス(左ペイン)を即時適用 ※保存状態は読込後に上書き
    Popup->>BM: getTree()
    Popup->>Alias: getAll()
    User->>Popup: 文字入力(1文字ごと) ※印字文字で検索ボックスへ復帰
...
**別名編集の終了導線**（`alias-editor-close`, 2026-08-16 追加）:

当初は `Escape` と空入力の `Enter` だけが終了手段で、**マウスだけでは終われなかった**。インライン編集（状態1d）には [キャンセル]/[保存] があるのに別名編集にだけ可視な終了手段が無い、という非対称を解消する。

| 操作 | 結果 | 入力途中の文字 |
|---|---|---|
| **右ペインの編集行の外を押す** | 閉じる。**その1クリックは飲む**（クリック先の操作を実行しない） | **確定する** |
| **左ペイン・ヘッダーを押す** | クリック先の操作（スコープ変更・検索入力等）を実行しつつ閉じる | **確定する** |
| **[完了] ボタン** | 閉じる | **確定する** |
| `Escape` | 閉じる | **破棄する** |
| `Enter`（入力が空） | 閉じる | — |

- **右ペインだけクリックを飲む理由**: 右ペインの押下先はほぼ結果行であり、通すとブックマークが開いてポップアップごと閉じる＝編集していた文脈を失う。別名編集にはアンドゥが無く、この事故のコストが大きい。意図が明確な2回目のクリックで開く。
- **左ペイン・ヘッダーを通す理由**: フォルダのスコープ変更・検索ボックスへの入力・「＋追加」は破壊的でなく、1クリックで完了しないほうが不自然。
- **`Escape` だけ破棄**にするのは、外側クリックが「確定」になったことで、打ちかけを捨てる明示的な手段が必要になるため。
- 別名編集中は**他行に指カーソルを出さない**（押しても閉じるだけで開かないため、機能とアフォーダンスを一致させる）。
- 副作用として、別の行の別名エリアを押して対象を移す操作は**2クリック**になる（1回目は現在の編集を閉じるだけ）。
...
> **フォルダスコープとの関係(U12・2026-08-08 確定)**: 「結果から消さずフォルダパス表示のみ更新」は **「すべて」表示・検索結果表示** に適用する(移動しても行は残り、フォルダパス表示だけが新しい場所へ更新される)。一方、**特定フォルダにスコープを当てている**とき(右ペインは当該フォルダの直下のみを表示)に、そのフォルダ**外**へブックマークを移動した場合は、スコープの直下判定(`node.parentId === scope.folderId`)により当該行は一覧から外れる。これは「そのフォルダから取り出した」という直感と一致するため、**スコープの直下表示ルールを優先する**。移動先を追い直したいときは左ペインで移動先フォルダにスコープを当て直す(移動自体はアンドゥで5秒間取り消せる)。
...
### UC-5c: フォルダの名前変更・新規作成(右クリックメニュー)

**フォルダの整理はすべて同じ右クリックメニューに集約する**(`folder-rename-create`, 2026-08-17)。UC-5b で導入したメニューへ「新しいフォルダ」「名前を変更」の2項目を足し、フォルダを作る・名前を直す・消すがポップアップ内で完結する(Chrome のブックマークマネージャーへ出る必要がない)。

```
┌──────────────────┐
│ 新しいフォルダ     │  ← 右クリックしたフォルダの「直下」に作る
│ 名前を変更        │  ← 最上位フォルダでは無効
├──────────────────┤  ← 破壊的操作を区切り線で離す
│ フォルダを削除     │  ← 最上位フォルダでは無効(UC-5b)
└──────────────────┘
```
...
sequenceDiagram
    ...
    User->>Tree: フォルダ行を右クリック
    Tree->>Popup: onFolderContextMenu(folderId, title, depth, x, y)
    Popup-->>User: メニュー表示(最上位は「名前を変更」「削除」が無効)

    alt 名前を変更
        User->>Popup: 「名前を変更」
        Popup-->>User: 入力ダイアログ(現在の名前を全選択して表示)
        User->>Popup: 新しい名前 + Enter
        Note over Popup: planRename → unchanged なら API を呼ばず閉じるだけ
        Popup->>BM: rename(id, title)
        Popup->>SE: 索引を再構築(配下全エントリの folderPath/nFolders が波及)
        Popup->>Popup: 左ペイン再取得
    else 新しいフォルダ
        User->>Popup: 「新しいフォルダ」
        Popup-->>User: 入力ダイアログ(空欄・確定ボタンは無効)
        User->>Popup: 名前 + Enter
        Popup->>BM: create({ title, parentId: 右クリックしたフォルダ })
        Popup->>SE: 索引を再構築(folderIdPaths に新フォルダを載せる)
        Popup->>Popup: 左ペイン再取得 → 親を展開 → 新フォルダをスコープに
    end
```

**整合ルール(UC-5b に追加)**:
- **名前変更のあとは検索索引を作り直す**。索引エントリは配下ブックマークごとに `folderPath`(行のパス表示)と `nFolders`(フォルダ名の照合語)を持ち、フォルダ名の変更は配下**全エントリ**へ波及する。`moveNode` のような部分更新では追随できない。
- **新規作成のあとも索引を作り直す**。エントリは増えないが、`SearchEngine` が別に持つ `folderIdPaths`(フォルダID → 祖先IDパス)を更新する必要がある。これを怠ると、作った直後のフォルダへブックマークを移した際にIDパス解決がフォールバック(自分1つだけのパス)へ倒れ、**祖先フォルダをスコープにしたときにそのブックマークが出ない**(配下判定はIDパスの包含で行うため)。
- **最上位フォルダは名前を変更できない**(Chrome がルート直下フォルダの `update` を拒否する)。一方**その直下へのフォルダ作成は拒否されない**ため制限しない。これにより「すべて」行にメニューを持たせなくても全階層にフォルダを作れる。
- **同名フォルダは許可する**。Chrome 自身が許可しており、ここだけ禁止すると操作が通らない理由を説明できない。スコープ判定はIDで行うため取り違えは起きない。
- **変更なしの確定では chrome API を呼ばない**(索引の再構築も起こさない)。前後の空白だけの差も「変更なし」と扱う。
- **入力ダイアログのキーは入力欄自身が処理して伝播を止める**。document リスナーへ素通しすると、日本語入力の**変換確定 `Enter`** が `panel:confirm` として解決され、変換を確定しただけでダイアログが閉じる。`isComposing` を見て変換中の `Enter` は無視する(`InlineEdit`/`AliasEditor` と同じ規律)。
- **矢印キーは横取りしない**(入力ダイアログのとき)。動かす候補が無いうえ、`preventDefault` すると入力欄内のカーソル移動まで潰れる。
- **名前変更・新規作成に即時アンドゥは設けない**。5秒アンドゥは破壊的操作(削除・移動)にのみ登録する規律に合わせる(ブックマークのリネームにも登録していない)。どちらも同じメニューから即座に取り消せる。
...
フォーカスは **検索ボックス / 右ペイン(ブックマーク一覧) / 左ペイン(フォルダツリー)** を行き来する。起動時は**既定で左ペイン**とし、保存された状態があればそれを復元する(機能13「ポップアップ状態の復元」)。
...
stateDiagram-v2
    state LIST {
        ...
        Result --> Search: 印字文字/Backspace / Escape
    }
    [*] --> LIST
    LIST --> FOLDER_TREE: 右ペインで ←
    FOLDER_TREE --> LIST: → / 印字文字/Backspace / Escape
...
> **行に紐づかない操作はフォーカス位置に依存しない(2026-08-15 是正)**: 上図のモード遷移規則(`PANEL` 等へは `LIST` からのみ入る)は**状態機械の内部規則**であり、ユーザーから見た到達可能性とは別。ショートカットは対象によって2種類に分かれる。
>
> | 種別 | 対象 | 有効なモード | 例 |
> |---|---|---|---|
> | **行に紐づく操作** | フォーカス中の結果行 | `LIST` のみ | `F2`/`Ctrl+E`(リネーム)、`Ctrl+;`(別名編集)、`Ctrl+M`(選択なし=単一移動)、`Delete`(選択なし=単一削除) |
> | **行に紐づかない操作** | チェック選択 / 対象なし | `LIST` + `FOLDER_TREE` | `Ctrl+D`(現在ページ登録)、`Ctrl+M`(選択あり=一括移動)、`Delete`(選択あり=一括削除)、`Ctrl/Cmd+A`(全件選択)、`Ctrl/Cmd+Z`(アンドゥ)、`Escape`(選択解除) |
>
> 後者は左ペインにフォーカスがあっても動作しなければならない。U11 で**起動直後の既定フォーカスが左ペイン(`FOLDER_TREE`)になった**ため、これを `LIST` 限定にすると「ポップアップを開いた直後に効かない」という主要導線の欠落になる(実際に `Ctrl+D`・一括移動・一括削除・全件選択の4つで発生していた)。実装上は `FOLDER_TREE` から `exitToList()` を経由して `PANEL` へ入るため、上図の遷移規則自体は維持される。
> なお、自前の文字入力 UI を持つモード(`INLINE_EDIT`/`ALIAS_EDIT`/`PANEL`/`DRAG`)ではいずれのショートカットも横取りしない(`isSearchFirstExempt` と同じ判定)。
>
> **例外: 検索ボックスにキャレットがあるときネイティブの意味を優先するキー**: `Delete`(文字の前方削除)と `Ctrl/Cmd+A`(テキスト全選択)は、`LIST` + 検索ボックスフォーカスのときだけ横取りしない。奪うとクエリの打ち直し・部分修正ができなくなるため(`←→` を検索ボックスで奪わないのと同じ理由)。左ペイン(`FOLDER_TREE`)は DOM フォーカスがテキスト入力ではないため、この例外は適用されない。
>
> **実装の所在(2026-08-15)**: 上記の判定は `pages/popup/src/hooks/modeMachine.ts` の `isShortcutEnabled(intent, ctx)` に集約されている(`ctx` = モード / フォーカス位置 / チェック選択件数 / 表示結果件数)。以前は `Popup.tsx` のキーハンドラ内に7通りの条件式として散在し、**「モード分岐のどのブロックに置くか」を誤ると該当ショートカットが静かに無効化される**構造だった(上記4件はいずれもこれが原因)。純粋関数へ切り出したことで、この組み合わせは単体テストで固定されている。
...
| `→` | キャレット移動 | — | 右ペインへ |
| `↑↓` | フォーカスが外れ、同時に選択行が1つ移動(端で循環) | 選択行の移動(端で循環) | 展開中のフォルダ間を移動(スコープが追従。端では何もしない) |
...
> **右ペインの `↑↓` を「端で循環」にした理由(2026-08-15 変更)**: 当初は「端では何もしない」(クランプ)としていたが、(1) 末尾行を見るために `↓` を長押しする必要があり `↑` 一発で末尾へ行けない、(2) キー入力に対して画面が一切変化せず「キーが効いていない」と誤解される、の2点から**循環**へ改めた。ランチャー系ツール・コマンドパレットの一般的な作法とも揃う。検索ボックスの `↑↓`(検索欄を離脱しつつ1つ移動)も同じ規則に従う — 同じ `LIST` モード内でフォーカス位置により端の挙動が変わる非対称を避けるため。**左ペイン(`FOLDER_TREE`)は循環させない**: 階層由来の並びで末尾から先頭(=「すべて」)へ飛ぶ意味が自明でなく、`Home` という一発復帰手段が別にあるため。インデックス計算は `pages/popup/src/hooks/listNavigationModel.ts` の `moveSelectionIndex` に集約する。

### 編集モードのキー挙動

| モード | ↑↓ | Enter | Escape | Tab / Shift+Tab |
|---|---|---|---|---|
| INLINE_EDIT | キャレット移動 | 確定→LIST | 破棄→LIST | タイトル⇄URL間の移動(確定・離脱はしない) |
| ALIAS_EDIT | 候補移動 | チップ確定 | 編集終了 | — |
| DRAG | — | — | ドラッグ中止 | — |
| PANEL | 候補移動 | 決定 | パネルを閉じる | — |

**INLINE_EDIT のフォーカスアウト確定**: フォーム全体(タイトル入力・URL入力・保存/キャンセルボタン)からフォーカスが外れたときのみ確定する。`Tab`/`Shift+Tab` によるフォーム内部の移動では確定・離脱しない(U10・PRD「未確定の論点」#5 の解消)。

**アンドゥのキーボード手段(U10)**: 削除直後のトースト表示中(5秒間)に限り `Ctrl/Cmd+Z` でアンドゥが発動する。トースト非表示時(保持なし)は `Ctrl/Cmd+Z` をネイティブの取り消しに委ねる(PRD「未確定の論点」#1 の解消)。
...
**共通ルール**: 編集モード(INLINE_EDIT / ALIAS_EDIT / PANEL)中を除き、印字文字または修飾なしの `Backspace` を打てば検索ボックスにフォーカスが戻る。左ペイン(FOLDER_TREE)もこの復帰の対象とする。編集中もリスト選択のハイライトを薄く保持する。
...
| 10 | フォルダ指定チップ右端の `✕`（クリックで絞り込み解除。design 1b） | チップは**表示専用**（`✕` を描かない）。スコープ解除は `Escape` の段階戻り、「すべて」復帰は `Home` が担う（U11） | チップは「操作主体ではない」（機能5）。押せない/紛らわしい `✕` を避け、解除はキーボードの段階戻りに一本化する |
| 11 | ツリー下部の「階層をたたむ ▴」固定バー（design 2b） | **実装しない**（全折りたたみは `←` 連打・`Home` で代替。U11） | 220px の縦領域を常時 34px 消費するコストに見合わない。キーボードで同等操作が可能 |
| 12 | スコープ中フォルダの開閉三角は `rgba(255,255,255,0.75)`・**背景なし**（design 2a・README L101） | スコープ中の行では**不透明の白**（非アクティブ時は accent 濃色）+ **静止状態でも半透明のピル背景**（`bg-white/20` / `bg-accent/15`、ホバーで `bg-white/35` / `bg-accent/25`）。非スコープ行は従来どおりホバーのみ | design mock は三角と名前が**1つの `div`** で、押下対象が行全体だった。U11 で押下対象を「三角＝展開」「名前＝スコープ選択」の2つへ分離した結果、三角が独立したボタンになり、**ホバー時にしか手掛かりが出ない＝キーボード操作中は押せると分からない**状態になった。塗りの範囲は mock どおり三角を含む行全体に戻したうえで、三角自身に静止状態の背景を与えて押下対象であることを示す |
| 13 | 行ホバーでファビコン位置がチェックボックスに変化し、それを押して複数選択する（design 1f・Gmail/Finder 挙動） | **ホバーでの段階表示を廃止**し、ヘッダーの「☑ 選択」トグルで入る**選択モード**を新設。選択モード中は**行のどこを押しても選択**（開かない）。チェックボックスは選択モード中のみ表示され、押下対象ではない（`selection-mode`） | 選択の唯一のマウス導線が 16×16px のチェックボックスに集中しており、**その周囲を押すと意図せずサイトが開いてポップアップが閉じる**（整理作業の文脈が失われる）誤操作が起きていた。「同じ行の中で数 px の差で『開く』と『選ぶ』が切り替わる」構造そのものが原因のため、モードで分離して行内から境界を無くした。押下対象も 16px から 56px の行全体へ広がる |
...
- 左ペイン(220px固定): 常時表示のフォルダツリー。「スコープ指定」と「ドロップ先」の2役。**常にどれか1つのフォルダにスコープが当たっており**(起動直後は「すべて」)、キーボードでフォーカスを移動すると同時にスコープが追従して右ペインが切り替わる。視認性のためフォルダ📁・名前はやや大きめ（名前14px / 📁16px / 行高34px）。
  - **U7 時点の暫定実装**: 各フォルダ行の右端に**フォルダ選択ボタン（📎クリップ）**を置き、押下でそのフォルダをスコープにする方式だった（デザインmockの件数表示に代わる意図的逸脱）。展開/折りたたみは「三角＋📁＋フォルダ名」全体の押下範囲。
  - **U11 での再設計（2026-07-28 決定）**: 上記の📎方式を廃止し、**行内の押下対象を2つに分離**する。(1) **展開三角のみ**を独立した押下対象にし、展開/折りたたみ専用にする（現状の記号 `▸`/`▾` は単体では直感的でないため、わかりやすい見た目に変更する。具体案は U11 計画時に決定）。(2) **「📁 フォルダ名」の名前部分**を押下対象にし、そのフォルダをスコープにする（現在の📎の役割を引き継ぐ）。子を持たないフォルダも名前部分は常に押下可能にする（現状は非ボタンの `<span>` だが、スコープ選択の対象にするため変更が必要）。**この再設計により📎ボタンは不要になり撤去する**。マウス操作(名前押下)とキーボード操作(フォーカス移動)は同じスコープ状態を更新する（二重管理にしない）。
...
**フォルダの操作導線**（`folder-delete` で導入し `folder-rename-create` で3項目へ。詳細は UC-5b / UC-5c）:

| 操作 | マウス | キーボード |
|---|---|---|
| スコープにする | フォルダ名のクリック | `↑↓` でフォーカス移動（スコープが追従） |
| 展開/折りたたみ | 展開三角のクリック | `Enter`（トグル） |
| 新しいフォルダ | 右クリック →「新しいフォルダ」 | 割り当てなし（メニューが唯一の導線） |
| 名前を変更 | 右クリック →「名前を変更」 | 割り当てなし（同上） |
| フォルダを削除 | 右クリック →「フォルダを削除」 | 割り当てなし（同上） |

- **フォルダ操作の導線は右クリックメニューに一本化する**。行内にホバーで現れるアイコンは置かない（結果行で ✎/🗑 を廃止したのと同じ判断。220px の左ペインでは押下対象がさらに小さくなる）。
- **右クリックは行の状態を変えない**（スコープを動かさない）。対象はメニュー項目を実行した時点で確定する。スコープが動くのは削除の実行後（親へ退避）と、新規作成の完了後（作ったフォルダへ移す）だけ。
- **最上位フォルダ（「ブックマーク バー」等）では名前変更・削除が無効**。Chrome がルート直下フォルダの更新/削除を拒否するため、項目は残したまま無効表示にして理由をツールチップで示す。**新しいフォルダは有効**。
- 「すべて」行・「さらに N 件…」行にはメニューを持たせない（フォルダではないため）。
...
| 操作メニュー | 編集/別名を編集/フォルダへ移動/削除 | **行の右クリック**で表示（`row-context-menu`）。ホバーの ✎/🗑 アイコンは廃止 |

- 行の高さ: 56px(約10行表示)。2段組（1段目=ファビコン+タイトル / 2段目=フォルダパス+別名チップ、`padding-left:26px`）の確定仕様は `docs/design/README.md`「右ペイン：結果行の共通仕様」を正とする。

**結果行の操作導線**（`row-context-menu` で改訂）:

| 操作 | マウス | キーボード |
|---|---|---|
| 開く | 行のクリック（別名チップ領域を除く） | `Enter` |
| インライン編集（タイトル/URL） | 右クリック →「編集」/ ダブルクリック | `F2` / `Ctrl+E` |
| 別名編集 | 右クリック →「別名を編集」/ **別名チップ領域のクリック** | `Ctrl+;` |
| フォルダへ移動 | 右クリック →「フォルダへ移動…」/ 左ペインへの D&D | `Ctrl+M` |
| 削除 | 右クリック →「削除」 | `Delete` |

- **ホバーで現れる ✎/🗑 アイコンは廃止した**。行の右端にだけ現れる小さな押下対象であり、隣を押すとサイトが開いてポップアップが閉じる誤操作の主因だったため（`selection-mode` で左のチェックボックスを廃止したのと同じ判断）。
- **別名チップ領域のクリックは存置する**。見えている要素そのものが押下対象で、ホバーで出入りしないため位置の取り違えが起きにくい。
- 右クリックは**行の状態を変えない**（開かない・フォーカス行を動かさない）。対象はメニュー項目を実行した時点で確定する。
- 選択モード中は行メニューを開かない（行操作は一括操作バーが担う）。

**選択モードとチェックボックスの表示**（`selection-mode` で改訂。旧「ホバーで段階表示」は**廃止**）:

行の押下は**モードによって意味が変わる**。行内に「開く/選ぶ」の境界を作らず、押下対象は常に行そのもの1つにする。

| 状況 | 行の表示 | 行を押したとき |
|---|---|---|
| 通常モード | ファビコンのみ（**ホバーしてもチェックボックスは出さない**） | 開く |
| 選択モード（0件選択） | 全行がチェックボックス（未選択 = 白枠）。ヘッダーは検索ボックスのまま | 選ぶ |
| 選択モード（1件以上選択） | 選択済み = accent 塗り + 白 `✓` + 行背景。ヘッダーが一括操作バーに切替 | 選ぶ |

- チェックボックスは**表示専用**（`pointer-events-none`）。クリック判定は行全体が受ける。枠は16px固定で、切替時にレイアウトが動かない。
- 選択モードへの入口は**ヘッダーの「☑ 選択」トグルボタン**。加えて `Ctrl/Cmd+クリック`・`Shift+クリック`・`Ctrl/Cmd+A` は通常モードからでも選択を発生させ、**同時に選択モードへ自動遷移**する（「選択があるのに通常モード」を作らない）。
- 選択モードの終了は「トグル再押下 / `Escape` / 一括操作バーの[選択解除] / 一括操作の完了後（自動）」の4経路。いずれも**選択のクリアを伴う**。
- 選択モード中は行の編集導線（✎/🗑 アイコン・ダブルクリック・別名エリアクリック・`F2`/`Ctrl+E`/`Ctrl+;`）を無効化する。`Delete`/`Ctrl+M` は選択0件では何も起きない（選ぶつもりの行へ破壊的操作を飛ばさない）。
...
├── popup_session               # PopupSession(フォーカス位置・スコープ・選択行・クエリ / 機能13)
...
- **起動時の先読み**: ポップアップ表示と並行して `getTree()` / `getAll()` を非同期取得し、フォーカスは**既定値を即座に当てる**(200ms以内)。保存状態(機能13)の復元は `storage.local` 読み込み後に上書きし、選択行の ID 解決は索引構築の完了を待つ。
