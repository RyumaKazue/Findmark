...
**定義**: 検索ボックス先頭に挿入され、検索を特定フォルダ配下に絞り込む要素(例 `📁 開発/chrome`)。

**説明**: フォルダ指定は常に1つ。状態を検索ボックス1箇所に集約することで「選択中フォルダ」という隠れた状態を持たず、整合性バグを構造的に防ぐ。**絞り込み範囲であり検索語ではない**ため、フォルダパスは検索語の照合対象から除外される。デフォルトでサブフォルダを含み、クリックで「直下のみ」にトグルできる。

**関連用語**: フォルダツリー、folderScope、別名チップ

### 検索ファースト

**定義**: フォルダ階層を辿るのではなく、起動即フォーカスの検索ボックスから絞り込む操作思想。

**説明**: Spotlight / Alfred 型のUI。編集モード中を除き、文字を打てば検索ボックスにフォーカスが戻る。ファイルマネージャ型(左でフォルダ選択→右に中身)とは異なり、右ペインは常にフォルダ横断のフラットリスト。

**関連用語**: フォルダチップ、モード状態遷移
...
| モード | 意味 | ↑↓ | Enter | Escape |
|-------|------|-----|-------|--------|
| LIST | 既定。検索結果を操作 | 選択移動 | 開く | 段階的に戻る |
...
**定義**: URLを正規化(フラグメント除去・末尾スラッシュ正規化等)してからハッシュ化した文字列。
