...
### 1a — 通常状態（検索前・初回起動時）
- **Purpose**: ポップアップを開いた直後（**保存された状態がない初回起動時**）。全ブックマークを閲覧。
- **前提**: 2回目以降は前回のフォーカス位置・スコープ・選択行・クエリが復元されるため（機能13）、必ずしもこの状態から始まるとは限らない。本状態は既定値（フォーカス=左ペイン / スコープ=「すべて」/ 選択行=先頭 / クエリ=空）の見え方を定義する。
...
- **左ペイン**: スコープは「すべて」（最上位の「すべて」がハイライト）。※スコープは常にどれか1つに当たっており、「選択なし」の状態は存在しない。初回起動時はここにキーボードフォーカスがある。
...
- 下にヒント「Enter で確定 / Backspace で直前のチップを削除」 11px `#9AA1AE`。右端に `N / 20` と **[完了] ボタン**（height 22, radius 6, 白地 + `border-line-input`）。※[完了] は `alias-editor-close` で追加。インライン編集（1d）に [キャンセル]/[保存] があるのに、別名編集にだけ押して終わる手段が無かったため。
- 他行は `opacity: 0.4`。**編集中は他行に指カーソルを出さない**（`cursor: default`）。押しても「編集を閉じるだけ」で開かないため、押せる見た目を残さない（`alias-editor-close`）。
- **閉じる操作**（`alias-editor-close` で拡張）:

  | 操作 | 結果 | 入力途中の文字 |
  |---|---|---|
  | 右ペインの編集行の外を押す | 閉じる。**その1クリックは飲む**（ブックマークを開かない） | 確定する |
  | 左ペイン・ヘッダーを押す | クリック先の操作を実行しつつ閉じる（1クリック） | 確定する |
  | [完了] | 閉じる | 確定する |
  | `Escape` | 閉じる | **破棄する** |
  | `Enter`（入力が空） | 閉じる | — |

  ※別の行の別名エリアを押して対象を移す操作は**2クリック**になる（1回目は現在の編集を閉じるだけ）。
...
| 操作 | 挙動 |
|---|---|
| ポップアップ表示 | フォーカスは**既定で左ペイン**（保存された状態があれば復元＝機能13）。スコープは既定「すべて」で、全ブックマークを**タイトル昇順**（仮想スクロール）で表示。※「検索ボックスに自動フォーカス」「更新日時降順」はいずれも不採用 |
| 文字入力 | インクリメンタル検索（debounce 120ms 目安）。タイトル・URL・別名を対象に部分一致。**別名の一致を最上位にランク付け** |
| フォルダ選択（**「📁 フォルダ名」部分のクリック** / 左ペインでのキーボード移動） | そのフォルダを**スコープ**にし、**直下のブックマークのみ**を右ペインに表示（サブフォルダは含めない）。ツリー側もハイライト。※旧仕様の「サブフォルダ配下も対象」は不採用。※U11再設計（2026-07-28）: 従来の📎クリップボタンでの選択を廃止し、フォルダ名部分のクリックに統合。子を持たないフォルダも選択対象にする |
| 検索ボックス先頭のフォルダチップ | **現在のスコープを可視化する表示要素**。チップ自体はスコープを操作する主体ではない（解除は `Escape` の段階戻り、「すべて」への復帰は左ペインの `Home`） |
| フォルダ行の右クリック | フォルダ操作メニュー（現在は「フォルダを削除」）。※`folder-delete` で追加。既定のブラウザメニューはポップアップ全体で抑止する（テキスト入力欄を除く）。メニューを開いただけではスコープは変わらない。最上位フォルダでは削除が無効表示。中身があるフォルダは件数付きの確認ダイアログを挟む |
| 三角クリック | 展開/折りたたみ。展開状態は永続化。※キーボードでの展開/折りたたみは `Enter` のトグル（`←→` はペイン移動に使うため不採用）。※U11再設計（2026-07-28）: 三角のみを独立した押下対象にし、フォルダ名部分（スコープ選択）と分離する。記号 `▸`/`▾` は単体では直感的でないため、わかりやすい見た目に変更する（具体案はU11計画時に決定） |
...
| 別名編集中に編集行の外を押す | 閉じる（入力途中の文字は確定）。**右ペインではその1クリックを飲む**（ブックマークを開かない）／左ペイン・ヘッダーはクリック先の操作も実行する。※`alias-editor-close` で追加 |
...
| `focusArea: 'result' \| 'folderTree'` | 印字文字 / 修飾なし `Backspace` / `Esc` | `'search'` | 印字文字/Backspace はそのまま検索ボックスへ適用される（ブラウザの既定動作が新フォーカス先に対して発火する挙動を利用） |
