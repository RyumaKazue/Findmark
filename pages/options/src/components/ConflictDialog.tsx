...
export const ConflictDialog = ({ existing, incoming, count, onResolve }: ConflictDialogProps) => {
  ...
  return (
    <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/30">
      <div
        role="dialog"
        aria-label={t('optionsConflictDialogLabel')}
        className="shadow-shell border-line w-[480px] rounded-lg border bg-white p-5">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-ink text-[14px] font-bold">{t('optionsConflictHeading')}</h2>
          <span className="text-ink-faint text-[11.5px]">{t('optionsConflictCount', String(count))}</span>
        </div>

        <div className="mb-4 grid grid-cols-2 gap-3">
          <div className="border-line rounded-md border p-3">
            <p className="text-ink-faint mb-1 text-[11px] font-bold">{t('optionsConflictExisting')}</p>
            <p className="text-ink truncate text-[13px] font-medium" title={existing.title}>
              {existing.title}
            </p>
            <p className="text-ink-soft mt-1 truncate text-[11.5px]">
              {existing.folderPath.join(' / ') || t('commonRootFolder')}
            </p>
          </div>
          <div className="border-accent bg-accent-bg rounded-md border p-3">
            <p className="text-accent-strong mb-1 text-[11px] font-bold">{t('optionsConflictIncoming')}</p>
            <p className="text-ink truncate text-[13px] font-medium" title={incoming.title}>
              {incoming.title}
            </p>
            <p className="text-ink-soft mt-1 truncate text-[11.5px]">
              {incoming.folderPath.join(' / ') || t('commonRootFolder')}
            </p>
          </div>
        </div>
